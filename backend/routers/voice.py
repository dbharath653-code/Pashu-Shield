...
async def process_voice_intent(
    ...
    if any(w in text for w in ["report", "sick", "ill", "disease", "आजारी", "तक्रार", "बीमार", "fever", "blister"]):
        ...
        triage = TriageEngine.evaluate(
            species=species,
            symptoms=extracted_symptoms if extracted_symptoms else ["Fever", "Loss of appetite"],
        ...
        action_payload = {
            ...
            "number_dead": dead_count,
            "symptoms": extracted_symptoms if extracted_symptoms else ["Fever", "Loss of appetite"],
            "district": current_user.district if current_user and current_user.district else "Pune",
            "village": current_user.village if current_user and current_user.village else "Shirur",
            "triage_risk_level": triage["risk_level"]
        }
        
        if lang == "mr":
            sym_str = ", ".join(extracted_symptoms) if extracted_symptoms else "ताप आणि चारा न खाणे"
            fulfillment = f"मी {affected_count} {species} साठी अहवाल तयार केला आहे ज्यांना लक्षणे आहेत: {sym_str}. जोखीम पातळी {triage['risk_level']} आहे. मी हा अहवाल दाखल करू का?"
        else:
            sym_str = ", ".join(extracted_symptoms) if extracted_symptoms else "fever and appetite loss"
            fulfillment = f"I have prepared a disease report for {affected_count} {species} with {sym_str}. Risk level assessed as {triage['risk_level']}. Shall I submit this report to the veterinary network?"
        ...
        requires_conf = False
        fulfillment = "Your livestock herd is up to date with FMD round 4. Next booster is due in 45 days." if lang != "mr" else "तुमच्या जनावरांचे एफएमडी लसीकरण पूर्ण आहे. पुढची लस ४५ दिवसांत देणे आवश्यक आहे."
        ...
        requires_conf = False
        fulfillment = "Opening laboratory tracking. Blood sample SMP-10231 is under testing (RT-PCR)." if lang != "mr" else "प्रयोगशाळा निकाल उघडत आहे. नमुना SMP-10231 ची चाचणी सुरू आहे."
